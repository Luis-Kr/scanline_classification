...
def split_data_into_features_labels(cfg, training_data, testing_data, logger):
    ...
    logger.info(f"Statistics to drop: {cfg.training.statistics}")
    drop_col = cfg.training.statistics.lower()
    cols_to_consider = ["_".join([cfg.training.col_to_consider, stat]) for stat in cfg.training.statistics_all]
    logger.info(f"Columns to consider: {cols_to_consider}")
    
    if drop_col == "none":
        logger.info("No column will be dropped")
        
        #stats_columns_train = training_data.columns[training_data.columns.str.contains(cols_to_consider, case=True)]
        x_train = training_data[cols_to_consider]
        #x_train = training_data[stats_columns_train]
        y_train = training_data['label']
        
        #stats_columns_test = testing_data.columns[testing_data.columns.str.contains(cols_to_consider, case=True)]
        x_test = testing_data[cols_to_consider]
        ...
        # Training data
        drop_col = cfg.training.col_to_consider + "_" + cfg.training.statistics
...
def validation_data_evaluation(cfg: DictConfig,
    ...
    drop_cols_init = ["x_median_nn", "y_median_nn", "x_perc2nd_nn", "y_perc2nd_nn", "x_perc98th_nn", "y_perc98th_nn", "segment_id"]
    ...
    for file_path in Path(file_paths_dir).glob('*.txt'):
        ...
        validation_data = data.drop(columns=drop_cols_init)
        ...
        if drop_col == "none":
            logger.info(f"Column that will be dropped: {drop_col}")
            x_test_subsampled = validation_data[cols_to_consider]
        ...
        cls_report_subs_frac_df['drop_statistics'] = cfg.training.statistics
        cls_report_subs_frac_df['column'] = cfg.training.col_to_consider
        ...
        cls_report_subs_frac_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_cls_report_subs_frac.csv")
        ...
        cls_report_subs_df['drop_statistics'] = cfg.training.statistics
        cls_report_subs_df['column'] = cfg.training.col_to_consider
        cls_report_subs_df['baseline_model'] = baseline_model
        cls_report_subs_df['filename'] = filename
        cls_report_subs_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_cls_report_subs.csv")
        ...
        cls_report_fr_df['drop_statistics'] = cfg.training.statistics
        cls_report_fr_df['column'] = cfg.training.col_to_consider
        cls_report_fr_df['baseline_model'] = baseline_model
        cls_report_fr_df['filename'] = filename
        cls_report_fr_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_cls_report_fr.csv")
        ...
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_confusion_matrix_sampled_subs_pcd.csv", cnf_matrix_subs_frac, delimiter=',', fmt='%u')
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_confusion_matrix_subs_pcd.csv", cnf_matrix_subs, delimiter=',', fmt='%u')
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.col_to_consider}col_{cfg.training.statistics}stat_confusion_matrix_fr_pcd.csv", cnf_matrix_fr, delimiter=',', fmt='%u')
