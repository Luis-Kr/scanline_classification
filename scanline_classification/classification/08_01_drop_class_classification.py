...
def evaluate_model(y_pred, y_test, label_names_list, logger):
    ...
    cls_report = classification_report(y_test, y_pred, digits=3, target_names=np.array(label_names_list), output_dict=True)
...
def training(cfg, logger):
    ...
    for i, (test_indices1, test_indices2) in enumerate(selected_files):
        ...
        logger.info(f"Training data shape: {training_data_subset.shape}")
        ...
        # Drop rows with a certain label
        training_data_subset = training_data_subset[training_data_subset['label'] != cfg.training.class_to_drop]
        training_data_subset['label'] = pd.Categorical(training_data_subset['label']).codes
        ...
        # Remove the class to drop from the label names
        label_names_list = [label_names[i] for i in range(6) if i != cfg.training.class_to_drop]
        logger.info(f"Training: labels that will be considered: {label_names_list}")
        ...
        for file_path in selected_files[i]:
            ...
            # Drop rows with a certain label
            test_data_subsampled = test_data_subsampled[test_data_subsampled['label'] != cfg.training.class_to_drop]
            pcd_fr = pcd_fr[pcd_fr[:, cfg.pcd_col.label] != cfg.training.class_to_drop]
            
            test_data_subsampled['label'] = pd.Categorical(test_data_subsampled['label']).codes
            pcd_fr[:, cfg.pcd_col.label] = pd.Categorical(pcd_fr[:, cfg.pcd_col.label]).codes
            ...
            # Create a new dataframe with segment_ids from validation_data_subsampled and y_pred
            x_training_pred_df = pd.DataFrame({'id': test_data_subsampled['segment_id'], 'y_pred': y_pred})
            
            # Create a new dataframe with segment_ids and ground truth label from pcd_fr
            fr_predicted_labels_df = pd.DataFrame({'id': pcd_fr[:, cfg.pcd_col.segment_ids], 'y_true': pcd_fr[:, cfg.pcd_col.label]})
            
            # Unfold labels
            fr_prediction_df = unfold_labels_df(x_training_pred_df, fr_predicted_labels_df)
            
            # Get validation data
            validation_pred = fr_prediction_df['y_pred'].values
            validation_test = fr_prediction_df['y_true'].values
            ...
            cls_report_fr_df['class_to_drop'] = cfg.training.class_to_drop
...
def unfold_labels_df(x_validation_pred_df: pd.DataFrame, fr_predicted_labels_df: pd.DataFrame):
    fr_prediction_df = fr_predicted_labels_df.merge(x_validation_pred_df, on="id", how="inner")

    return fr_prediction_df
...
def validation(cfg, model, attributes_to_consider, label_names, logger):
    ...
    for i, file_path in enumerate(Path(cfg.training.validation_data_dir).glob('*.txt')):
        ...
        # Drop rows with a certain label
        validation_data_subsampled = validation_data_subsampled[validation_data_subsampled['label'] != cfg.training.class_to_drop]
        pcd_fr = pcd_fr[pcd_fr[:, cfg.pcd_col.label] != cfg.training.class_to_drop]
        
        validation_data_subsampled['label'] = pd.Categorical(validation_data_subsampled['label']).codes
        pcd_fr[:, cfg.pcd_col.label] = pd.Categorical(pcd_fr[:, cfg.pcd_col.label]).codes
        ...
        # Create a new dataframe with segment_ids from validation_data_subsampled and y_pred
        x_validation_pred_df = pd.DataFrame({'id': validation_data_subsampled['segment_id'], 'y_pred': y_pred})
        
        # Create a new dataframe with segment_ids and ground truth label from pcd_fr
        fr_predicted_labels_df = pd.DataFrame({'id': pcd_fr[:, cfg.pcd_col.segment_ids], 'y_true': pcd_fr[:, cfg.pcd_col.label]})
        
        # Unfold labels
        fr_prediction_df = unfold_labels_df(x_validation_pred_df, fr_predicted_labels_df)
        ...
        # Remove the class to drop from the label names
        label_names_list = [label_names[i] for i in range(6) if i != cfg.training.class_to_drop]
        logger.info(f"Validation: labels that will be considered: {label_names_list}")
        ...
        cls_validation_report_df['class_to_drop'] = cfg.training.class_to_drop
