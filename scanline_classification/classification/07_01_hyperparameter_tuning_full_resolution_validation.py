...
#     if cls_testing_results_path.exists():
...
#     # Write or append 
#     pd.DataFrame(cls_report_fr_dfs).to_csv(cls_testing_results_path, mode=mode, header=header, index=True)
    
    
#     # Check if one of the files exists
#     if cls_validation_results_path.exists():
#         mode = 'a'  # append if file exists
#         header = False  # don't write header if appending
#     else:
#         mode = 'w'  # write if file doesn't exist
...
#     # Write or append 
#     pd.DataFrame(cls_validation_report_df_out).to_csv(cls_testing_results_path, mode=mode, header=header, index=True)

#     time_end_main = time.time()
#     total_time = time_end_main - time_start_main
#     logger.info(f">>> Total time for the parameter test: {total_time:.2f} <<<")
