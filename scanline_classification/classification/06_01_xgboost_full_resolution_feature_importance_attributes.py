...
def split_data_into_features_labels(cfg, training_data, testing_data, logger):
    # Split the data into features and labels
    logger.info("Splitting the data into features and labels...")
    
    logger.info(f"cfg.training.drop_col: {cfg.training.drop_col}")
    drop_col = cfg.training.drop_col.lower()
    
    if drop_col == "none":
        logger.info("No column will be dropped")
        
        stats_columns_train = training_data.columns[training_data.columns.str.contains(cfg.training.statistics, case=True)]
        x_train = training_data[stats_columns_train]
        y_train = training_data['label']
        
        stats_columns_test = testing_data.columns[testing_data.columns.str.contains(cfg.training.statistics, case=True)]
        x_test = testing_data[stats_columns_test]
        y_test = testing_data['label']
        
        logger.info(f"Column shape {x_train.columns.shape}")
        logger.info(f"Column names {x_train.columns}")
        ...
        # Training data
        drop_col = cfg.training.drop_col + "_" + cfg.training.statistics
        logger.info(f"Column that will be dropped: {drop_col}")
        stats_columns1 = training_data.columns[training_data.columns.str.contains(cfg.training.statistics, case=True)]
        ...
        # Testing_data
        stats_column2 = testing_data.columns[testing_data.columns.str.contains(cfg.training.statistics, case=True)]
...
def validation_data_evaluation(cfg: DictConfig,
    ...
    for file_path in Path(file_paths_dir).glob('*.txt'):
        ...
        if drop_col == "none":
            statistic_columns = validation_data.columns[validation_data.columns.str.contains(cfg.training.statistics, case=True)]
            x_test_subsampled = validation_data[statistic_columns]
            ...
            statistic_columns = validation_data.columns[validation_data.columns.str.contains(cfg.training.statistics, case=True)]
            x_test_subsampled = validation_data[statistic_columns]
            x_test_subsampled = x_test_subsampled.drop(columns=drop_col)
            y_test_subsampled = validation_data['label']
            baseline_model = 0
        ...
        cls_report_subs_frac_df['statistics'] = cfg.training.statistics
        cls_report_subs_frac_df['drop_col'] = cfg.training.drop_col
        cls_report_subs_frac_df['baseline_model'] = baseline_model
        ...
        cls_report_subs_frac_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_cls_report_subs_frac.csv")
        ...
        cls_report_subs_df['statistics'] = cfg.training.statistics
        cls_report_subs_df['drop_col'] = cfg.training.drop_col
        cls_report_subs_df['baseline_model'] = baseline_model
        cls_report_subs_df['filename'] = filename
        cls_report_subs_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_cls_report_subs.csv")
        ...
        cls_report_fr_df['statistics'] = cfg.training.statistics
        cls_report_fr_df['drop_col'] = cfg.training.drop_col
        cls_report_fr_df['baseline_model'] = baseline_model
        cls_report_fr_df['filename'] = filename
        cls_report_fr_df.to_csv(classification_report_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_cls_report_fr.csv")
        
        cls_report_fr_dfs.append(cls_report_fr_df)
        
        logger.info(f"Saving the results for {filename}...")
        
        cnf_matrix_dir = Path(cfg.training.output_dir) / "confusion_matrices"
        cnf_matrix_dir.mkdir(parents=False, exist_ok=True)
        
        # Save the confusion matrix for the sampled subsampled pcd
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_confusion_matrix_sampled_subs_pcd.csv", cnf_matrix_subs_frac, delimiter=',', fmt='%u')
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_confusion_matrix_subs_pcd.csv", cnf_matrix_subs, delimiter=',', fmt='%u')
        np.savetxt(cnf_matrix_dir / f"{filename}_{cfg.training.drop_col}col_{cfg.training.statistics}stat_confusion_matrix_fr_pcd.csv", cnf_matrix_fr, delimiter=',', fmt='%u')
...
def main(cfg: DictConfig):
    ...
    # Split the data into features and labels
    x_train, x_test, y_train, y_test, sample_weights = split_data_into_features_labels(cfg, training_data, testing_data, logger)
    ...
    pd.DataFrame(cls_report_subs_frac_dfs).to_csv(cls_results_subs_frac_path, index=True, header=True)
    pd.DataFrame(cls_report_subs_dfs).to_csv(cls_results_subs_path, index=True, header=True)
    pd.DataFrame(cls_report_fr_dfs).to_csv(cls_results_fr_path, index=True, header=True)
